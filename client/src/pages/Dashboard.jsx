...
// `
  ...
  // (Xóa các hàm getPidEc và setifo cũ đi)

useEffect(() => {
    const fetchDataDirectly = async () => {
      // Đảm bảo state.contract đã được tải
      if (!state.contract) {
        console.log("Contract is not loaded yet");
        return;
      }

      try {
        // GỌI TRỰC TIẾP HÀM candidateList() TỪ CONTRACT
        const candidateArray = await state.contract.candidateList();
        
        // Vì component 'List' cần data theo cấu trúc của The Graph,
        // chúng ta phải 'giả lập' cấu trúc data đó.
        const simulatedGraphData = {
          candidates: candidateArray.map(c => ({
            _name: c.name,
            _party: c.party,
            // Chuyển BigInt từ Solidity sang Number
            _candidateId: Number(c.candidateId) 
          })),
          voters: [], // Có thể để rỗng nếu không dùng
          ecWinners: [] // Có thể để rỗng nếu không dùng
        };

        setinfo(simulatedGraphData); // Cập nhật data

        // Cập nhật state 'run'
        if (simulatedGraphData.candidates.length === 0) {
          setRun(false);
        } else {
          setRun(true);
        }
        ...
        console.error("Error fetching candidateList from contract:", error);
        toast.error("Failed to fetch candidates from contract.");
        setRun(false);
      }
    };
    ...
    // Chúng ta cần 'state.contract' làm dependency
    // để useEffect chạy lại khi contract được tải
  }, [state.contract, setinfo]);
